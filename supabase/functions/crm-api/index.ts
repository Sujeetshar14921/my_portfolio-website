...
const FROM_EMAIL = Deno.env.get("FROM_EMAIL") || "newsletter@sujeetsharma.in";
...
function generateICS(opts: { title: string; agenda: string; dateStr: string; timeStr: string; duration: number; meetingUrl: string }): string {
  const dt = new Date(`${opts.dateStr}T${normalizeTime(opts.timeStr)}`);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "verify_lead") {
      ...
      if (!lead) return json({ error: "Invalid or expired verification link." }, 404);
      if (lead.email_verified) return json({ message: "Your email is already verified.", name: lead.name });

      await getSupabase()
        .from("contact_submissions")
        .update({ email_verified: true, verification_token: null, status: "verified" })
        .eq("id", lead.id);
      ...
      return json({ message: "Your email has been verified successfully!", name: lead.name });
    }
    ...
    if (action === "schedule_meeting") {
      ...
      const dt = new Date(`${meetingDate}T${normalizeTime(meetingTime)}`);
